		...
		if (g_main_total)
		{
...

			{
				...
				if (dc)
				{
					...
					if (df)
					{
						prof_log.success("\t  of those      %.1f%% SPURIOUS (line unchanged; reservation-table aliasing, not a real conflict)",
							(spurious - g_putllc_spurious_prev) * 100.0 / df);
					}
				}
				...
				g_putllc_spurious_prev = spurious;
			}
