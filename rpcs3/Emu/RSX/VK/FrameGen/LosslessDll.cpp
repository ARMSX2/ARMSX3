...
[[nodiscard]] bool WriteShaderCache(const std::string& path, const CacheHeader& header,
    ...
    return fs::write_file(path, fs::rewrite, blob.data(), blob.size());
}
...
[[nodiscard]] bool StatSource(const std::string& path, u64& out_size, u64& out_mtime) {
    // PORT: FILESYSTEM_STAT_DATA -> fs::stat_t, whose fields are lower-case.
    fs::stat_t sd{};
    if (path.empty() || !fs::get_stat(path, sd)) {
        return false;
    }
    out_size = static_cast<u64>(sd.size);
    out_mtime = static_cast<u64>(sd.mtime);
    return true;
}
...
[[nodiscard]] bool ReadShaderCache(const std::string& path, u64 source_size, u64 source_hash,
    ...
    // PORT: FileSystem::ReadBinaryFile -> fs::file. The optional is kept so the rest of the
    // function reads unchanged: an unopenable file yields nullopt exactly as before.
    std::optional<std::vector<u8>> blob;
    if (fs::file f{path}) blob = f.to_vector<u8>();
    if (!blob) {
        return false;
    }
...
[[nodiscard]] LosslessStatus ReadImageFile(const std::string& path,
    ...
    std::optional<std::vector<u8>> image;
    if (fs::file f{path}) image = f.to_vector<u8>();
    if (!image) {
...
std::string GetLosslessDllPath() {
    // PCSX2 stored the path in GSConfig; RPCS3 keeps it with the other framegen settings.
    return g_cfg.video.frame_generation_dll_path.to_string();
}

std::string GetShaderCachePath() {
    // Same directory the previous implementation used, so an existing cache is found.
    return fs::get_cache_dir() + CACHE_FILE_NAME;
}
...
LosslessStatus LoadShaderModules(ShaderModules& out_modules, bool allow_fp16, bool prefer_fp16) {
    ...
    // PORT: GSXXH3_64bits -> FNV-1a. This only has to notice that Lossless.dll changed
    // since the cache was written, so any stable 64-bit hash serves; RPCS3 vendors no XXH.
    u64 source_hash = 0xcbf29ce484222325ull;
    for (const u8 b : image) { source_hash = (source_hash ^ b) * 0x100000001b3ull; }
