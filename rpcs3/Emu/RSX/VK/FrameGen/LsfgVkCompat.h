...
namespace Vulkan
{
	...
	/// Adapter over RPCS3's vk::render_device presenting the three queries the ported code asks
	/// for. Ported from the PCSX2 version, which wrapped GSDeviceVK; the questions are the same
	/// and only the object answering them changed.
	class Device
	{
	public:
		explicit Device(const ::vk::render_device* dev);
