...
namespace
{
	// ARMSX3: RPCSX added a vector-backed fs::virtual_dir to their Utilities/File.h.
	// Upstream only ships the abstract fs::dir_base (upstream's own iso_dir
	// implements read()/rewind() itself). Rather than diverge in a core header
	// for one helper, keep it local -- this file is the only user.
	class vector_dir final : public fs::dir_base
	{
		std::vector<fs::dir_entry> m_entries;
		std::size_t m_pos = 0;

	public:
		explicit vector_dir(std::vector<fs::dir_entry> entries)
...
std::unique_ptr<fs::dir_base> iso_dev::open_dir(const std::string& path)
{
	...
	return std::make_unique<vector_dir>(std::move(result_items));
}
