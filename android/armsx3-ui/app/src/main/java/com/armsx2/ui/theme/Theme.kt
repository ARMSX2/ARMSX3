...
object ThemePreferences {
    ...
    // ARMSX3 ships PURPLE by default -- the PS3 house colour. ARMSX2 defaulted to
    // System, which resolves to the blue Night scheme and made the whole app look
    // like ARMSX2. Users can still pick anything in Settings; this is only the
    // value a fresh install starts on.
    val mode = mutableStateOf(ThemeMode.Purple)
    ...
    fun load() {
        ...
        val stored = MainActivityRuntime.prefs.getString(PreferenceKey, ThemeMode.Purple.name)
        mode.value = ThemeMode.entries.firstOrNull { it.name == stored } ?: ThemeMode.Purple
        loadCustomColor()
        loadOledBase()
    }
