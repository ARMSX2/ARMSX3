...
val EN: Map<String, String> = mapOf(
    ...
    "adv.accurateCacheLine.description" to "Emulates 128-byte cache line stores precisely. Needed by some games for correct memory behaviour; costs performance.",
    ...
    "adv.ppuNan.description" to "Emulates PowerPC's exact handling of not-a-number values in vector maths. Needed by a few games that would otherwise show broken geometry.",
    "adv.accurateDfma.label" to "Accurate DFMA",
    "adv.accurateDfma.description" to "Uses exact double-precision fused multiply-add. On by default; turning it off is faster but can drift in physics-heavy games.",
    ...
    "memcard.delete.body" to "Remove \"%s\" from ARMSX3?\n\nThis deletes the card that ARMSX3 keeps in its own memory-card folder. If you imported it from a file on your device, that original file is not touched.",
    ...
    "patches.bundled.header" to "Bundled with ARMSX3",
    "patches.bundled.explain" to "These ship inside ARMSX3 and are applied to this game automatically — you didn't install them. Ones with no name are always on and can't be switched off individually.",
    ...
    "perf.fix.compatPatches.desc" to "Applies the per-game compatibility patches bundled with ARMSX3 — the fixes some games need to boot or render correctly. Same idea as GameDB Fixes above, and safe to leave on. Does NOT enable widescreen or cheats; those live in the Patches screen. Takes effect on the next boot.",
    ...
    "perf.fix.gamedbFixes.desc" to "Master switch for the compatibility fixes below, plus the automatic per-game fixes from ARMSX3's game database. Leave on unless troubleshooting.",
    ...
    "perf.configDb.title" to "Recommended Settings",
    ...
    "renderer.importTexturePack.bootFirst" to "Boot a game first so ARMSX3 knows the serial.",
    ...
    "renderer.forceCpuBlit.description" to "Do RSX surface-to-surface copies on the CPU instead of the GPU. Slower, but fixes games where blitted content shows up corrupted or missing.",
    ...
    "setup.bios.multipleFound" to "Firmware ready.",
    ...
    "setup.step.appData.description.play" to "Where PS3 save data, save states, configs and shader caches are stored. Internal uses your main device storage; SD Card uses a memory card if one is present. (Games are added separately.)",
