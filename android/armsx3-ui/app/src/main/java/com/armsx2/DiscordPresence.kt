...
object DiscordPresence {
    ...
    private fun raPresence(): String {
        ...
        val presence = runCatching { com.armsx3.NativeApp.getRichPresence().orEmpty() }
            .getOrDefault("").trim()

        val badge = runCatching {
            val items = com.armsx2.ui.achievements.parseAchievementItems(
                com.armsx3.NativeApp.getAchievementsJSON().orEmpty(),
            )
            if (items.isEmpty()) return@runCatching ""
            val hardcore = com.armsx3.NativeApp.isHardcoreMode()
    ...
    /** Serial last published, so a game change re-publishes and a poll does not. */
    private var lastPresenceSerial: String? = null
    ...
    private fun startPolling() {
        ...
        pollJob = scope.launch {
            ...
            while (true) {
                ...
                val wasConnected = status.value == CONNECTED
                status.value = s

                // Publish presence on the CONNECTED edge, and whenever the running
                // game changes. pushPresence() had no callers at all, so rich
                // presence was never sent -- the client connected and then sat
                // there advertising nothing.
                val nowGame = MainActivityRuntime.currentGame.value?.serial.orEmpty()
                if (s == CONNECTED && (!wasConnected || nowGame != lastPresenceSerial)) {
                    lastPresenceSerial = nowGame
                    pushPresence()
                }
