...
object BatteryWatcher {
    ...
    // Default OFF: a handheld running an emulator sits near the warning
    // thresholds most of the time, so the banners fire constantly and read as
    // noise rather than information. Still available in App settings.
    val enabled = androidx.compose.runtime.mutableStateOf(false)

    fun load() {
        runCatching {
            enabled.value = com.armsx2.runtime.MainActivityRuntime.prefs.getBoolean(PREF_KEY, false)
        }
    }
