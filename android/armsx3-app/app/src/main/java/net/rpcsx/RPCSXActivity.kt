...
class RPCSXActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ARMSX3: sustained performance is a WINDOW flag, so it has to be set
        // on the emulator activity -- setting it on MainActivity would do
        // nothing for the session that actually needs it.
        AndroidPerformance.applySustained(this)
        // Pause the launcher music for the whole session; the emulator owns
        // audio from here.
        LibraryMusic.stop(this)

        // ARMSX3: the core's home menu holds savestates, trophies, screenshot,
        // restart and exit. openHomeMenu() was declared but never called, so
        // none of it was reachable.
        binding.homeMenu.setOnClickListener {
            MenuSfx.play(MenuSfx.Event.MENU_OPEN)
