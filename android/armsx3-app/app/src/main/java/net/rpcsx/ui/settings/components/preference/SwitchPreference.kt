...
fun SwitchPreference(
    ...
    // ARMSX3: every settings toggle in the app funnels through here, so this one
    // hook gives the whole generated settings tree its on/off blips.
    val onValueUpdated: (Boolean) -> Unit = { newValue ->
        MenuSfx.play(if (newValue) MenuSfx.Event.TOGGLE_ON else MenuSfx.Event.TOGGLE_OFF)
        onClick(newValue)
    }
    RegularPreference(
        title = title,
