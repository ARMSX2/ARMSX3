...
object ControllerSkinStore {
    ...
    fun applyForSerial(ctx: Context, serial: String?) {
        ...
        val raw = if (eff != null)
            GeneralSettings.raw.getString(KEY_ACTIVE_GAME_PREFIX + eff, null)
                ?: GeneralSettings.raw.getString(KEY_ACTIVE, null)
        else GeneralSettings.raw.getString(KEY_ACTIVE, null)
    ...
    fun hasGameOverride(serial: String?): Boolean {
        ...
        return GeneralSettings.raw.contains(KEY_ACTIVE_GAME_PREFIX + eff)
    }
    ...
    fun clearGameOverride(ctx: Context, serial: String) {
        ...
        GeneralSettings.raw.edit().remove(KEY_ACTIVE_GAME_PREFIX + serial).apply()
    ...
    private fun ensureLoaded(ctx: Context) {
        if (loaded) return
        val saved = GeneralSettings.raw.getString(KEY_ACTIVE, null)
    ...
    /** Eagerly hydrate the saved selection at app start (MainActivity.onCreate) so the in-game
    ...
    fun setActive(ctx: Context, id: String?, serial: String? = null) {
        ensureLoaded(ctx)
        val eff = serial?.takeIf { it.isNotEmpty() }
        GeneralSettings.raw.edit().apply {
    ...
    fun delete(ctx: Context, id: String) {
        ...
        GeneralSettings.raw.edit().apply {
            if (GeneralSettings.raw.getString(KEY_ACTIVE, null) == id) remove(KEY_ACTIVE)
            GeneralSettings.raw.all.keys
                .filter { it.startsWith(KEY_ACTIVE_GAME_PREFIX) }
                .filter { GeneralSettings.raw.getString(it, null) == id }
                .forEach { remove(it) }
        }.apply()
        applyForSerial(ctx, activeSerial.value)
    }
     ...
     *  download path ([net.rpcsx.SkinRepo]).
     *
