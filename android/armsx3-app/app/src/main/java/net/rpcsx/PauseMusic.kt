...
object PauseMusic {
    ...
    fun load() {
        enabled.value = GeneralSettings.raw.getBoolean(EnabledKey, true)
        volumePercent.value = GeneralSettings.raw.getInt(VolumeKey, DefaultVolumePercent)
        customName.value = GeneralSettings.raw.getString(CustomNameKey, null)
    }

    fun set(context: Context, value: Boolean) {
        enabled.value = value
        GeneralSettings.raw.edit { putBoolean(EnabledKey, value) }
        ...
        if (!value) {
            stop()
        } else if (RPCSX.state.value == EmulatorState.Paused ||
            RPCSX.state.value == EmulatorState.Frozen
        ) {
            start(context)
        }
    }
    ...
    fun setVolume(percent: Int) {
        ...
        GeneralSettings.raw.edit { putInt(VolumeKey, p) }
    ...
    fun setCustomTrack(context: Context, uri: android.net.Uri, displayName: String): Boolean {
        ...
        if (ok) {
            ...
            GeneralSettings.raw.edit { putString(CustomNameKey, displayName) }
            restart(context)
        }
        return ok
    }
    ...
    fun clearCustomTrack(context: Context) {
        ...
        GeneralSettings.raw.edit { remove(CustomNameKey) }
        restart(context)
    }
