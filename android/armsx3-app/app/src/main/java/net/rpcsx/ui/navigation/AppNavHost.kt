...
import net.rpcsx.ui.skins.SkinsScreen

import net.rpcsx.ui.about.AboutScreen

import net.rpcsx.ui.settings.PerformanceSettingsScreen

import net.rpcsx.ui.setup.SetupScreen
...
fun AppNavHost() {
    ...
    NavHost(
        ...
        // ARMSX3: first run lands on setup instead of an empty games grid with
        // no hint that PS3 firmware is even required. Once completed the flag
        // is persisted, so this only fires once (or again via Settings -> reset).
        startDestination = if (SetupRepository.completed.value) "games" else "setup"
    ) {
        composable(
            route = "setup"
        ) {
        ...
        composable(
            ...
            GamesDestination(
                ...
                drawerState,
                navigateToPerformance = { navigateTo("performance") },
                navigateToAbout = { navigateTo("about") },
                navigateToSkins = { navigateTo("skins") }
            )
        }
        ...
        composable(
            ...
            SkinsScreen(navigateBack = navController::navigateUp)
        }

        // ARMSX3: Android-only power controls. Emulator-side perf settings
        // (frame limit, thread scheduler, ...) stay in the generated tree.
        composable(
            route = "performance"
        ) {
        ...
        drawerContent = {
            ModalDrawerSheet {
                Column(
                    ...
                    // ARMSX3: About is a real screen now (version + GitHub /
                    // Discord / site links + the What's New changelog off the
                    // Releases feed), not a copy-to-clipboard version dialog.
                    NavigationDrawerItem(
