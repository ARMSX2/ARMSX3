...
import net.rpcsx.ui.home.LibraryWaveBackground

import net.rpcsx.LibraryMusic
...
import net.rpcsx.CoverRepository
import java.io.File
    ...
    Column {
        ...
        Card(
                ...
                .combinedClickable(onClick = click@{
                    ...
                    if (FirmwareRepository.version.value == null) {
                        ...
                        if (game.findProgress(GameProgressType.Compile) != null) {
                            ...
                            // ARMSX3: launch blip, and hand the library music
                            // off before the emulator takes the audio device.
                            MenuSfx.play(MenuSfx.Event.SELECT)
                            LibraryMusic.stop(context)
            ...
            Box(
                ...
                // ARMSX3: prefer a downloaded PS3 cover over the game's own
                // ICON0.PNG. Covers are fetched lazily per title and cached, so
                // the first paint falls back to ICON0 and swaps in when it lands.
                // VSH keeps ICON0 -- it is not a retail title and has no cover.
                val titleId = remember(game.info.path) {
                    CoverRepository.titleIdFromPath(game.info.path)
                }
                val coverPath = if (titleId != null && game.info.name.value != "VSH") {
                    CoverRepository.coverState(titleId).value
                } else null

                LaunchedEffect(titleId) {
                    if (titleId != null && game.info.name.value != "VSH") {
                        CoverRepository.fetch(titleId)
                    }
                }
                ...
                if (coverPath != null) {
                    Row(
                        ...
                        AsyncImage(
                            ...
                            // aldostools covers are 260x300. Pinning the tile to
                            // their real aspect is what stops every cover being
                            // letterboxed inside a differently-shaped box.
                            modifier = Modifier
                                .fillMaxWidth()
                                .aspectRatio(CoverRepository.COVER_ASPECT)
                        )
                    }
...
fun GamesScreen() {
    ...
    val checkForUpdates = suspend {
        // ARMSX3: DO NOT check RPCSX's update channels.
        //
        // Upstream RPCSX ships no core in the APK and downloads one from
        // github.com/RPCSX/rpcsx(-build) on first run, then offers updates from
        // the same place. We bundle our own core (libarmsx3-core.so, built from
        // upstream RPCS3 with our patches), so those prompts were both wrong and
        // actively destructive: accepting one replaces our core with RPCSX's
        // completely different build, and the UI updater would install RPCSX's
        // APK over ARMSX3.
        //
        // ARMSX3's own update mechanism goes through ArmsxLinks.REPO instead.
    ...
    // ARMSX3: animated library backdrop, ported from ARMSX2. Drawn on the 2D
    // Canvas rather than GLES3 so it runs on any GPU (ARMSX2 keeps a GLES3 XMB
    // mesh with this as the fallback; we ship only the portable one). Sits
    // behind the grid with a scrim so covers and text stay readable.
    ...
    } // ARMSX3: closes the backdrop Box
}
