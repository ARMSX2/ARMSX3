...
object GitHub {
    ...
    data class Release(
        ...
        // ARMSX3: the What's New screen renders these. Defaulted so the existing
        // update-channel parsing (which only wants name+assets) is unaffected,
        // and json is configured with ignoreUnknownKeys anyway.
        val tag_name: String = "",
        val body: String = "",
    ...
    /**
     * Full release objects (name, tag, body, date) for the What's New screen.
     *
     * Distinct from [fetchReleases], which flattens to (name, firstAssetUrl)
     * pairs for the update-channel pickers and throws the changelog away.
     */
    suspend fun fetchReleaseNotes(repoUrl: String): FetchResult = withContext(Dispatchers.IO) {
        val repoPath = repoUrl.removePrefix(server)
        val apiUrl = "${apiServer}repos/$repoPath/releases"

        when (val response = get(apiUrl)) {
            is GetResult.Error ->
                FetchResult.Error("Failed to fetch releases: ${response.code} ${response.message}")

            is GetResult.Success -> try {
                val releases: List<Release> = json.decodeFromString(
                    ListSerializer(Release.serializer()), response.content
                )
                FetchResult.Success(releases)
            } catch (e: Exception) {
                FetchResult.Error("Parsing error: ${e.message}")
            }
        }
